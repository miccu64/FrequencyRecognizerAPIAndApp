...
public class ProcessSound {
    ...
    public ProcessSound(AudioFormat _format, int _len) {
        format = _format;
    ...
    private void doWindowing(float[] bufFloat) {
        ...
        for (int i=0; i<bufFloat.length; i++) {
            //bufFloat[i] = (float) (bufFloat[i] * Math.pow(Math.sin((Math.PI * i)/len), 2));
    ...
    private float[] bytesToFloat(byte[] bytes) {
        ...
        for (int pos = 0; pos < bytes.length / frameSize; pos++) {
            ...
            for (int part = 0; part < frameSize; part++) {
                ...
                sample += help << (8 * (frameSize - part - 1));
            }
            res[pos] = sample;
        }
        return res;
